...
class Maze {
	...
	getShortestPath() {
		...
		let path_sections = [...this.entrances.values()].map(([x, y]) => [
			makePath({ x, y }),
		]);
		while (path_sections.some(path_section => path_section.some((path) => !path.at_end))) {
			let new_path_sections = Array(4).fill();
			for (let p = 0; p < path_sections.length; p++) {
				const paths = path_sections[p];
				 ...
				 */
				new_path_sections[p] = [...pruned_paths.values()].sort((a, b) => a.steps - b.steps);
				// if (path_sections[p].length > 100) {
				// 	path_sections[p] = path_sections[p].slice(0, Math.ceil(path_sections[p].length * 0.68));
				// }
			}
			path_sections = new_path_sections;
		}

		return path_sections.map(s => s[0]);
	}
}
