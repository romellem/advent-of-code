 ...
 * @property {?(x, y) => unknown} defaultFactory - Defaults to returning 0 for new coords
 * @property {?Record<unknown, string>} string_map - Map grid values to strings.
 * @property {?String|unknown[][]} load - Initial grid to load. Can be a "2D" string (string with new lines), or a "2D" array.
 * @property {?(v) => unknown} parseAs - When `load` is defined, this parses the cell in the split string. Defaults to `String`.
 */
